...
Upload file báo cáo hoa hồng Shopee (`.csv`, `.xlsx`, `.xls`), **gộp theo `ID đơn hàng`**, rồi **upsert** vào bảng `public.affiliate_commission_orders`. Sau đó **đồng bộ phân tầng** vào `public.affiliate_commission_order_splits` (migration **`006`**, **`008`** tắt RLS nếu cần): mỗi đơn có dòng **chủ tool** (`id_zl` cố định) **25%** `net_affiliate_commission` và dòng **đại lý** **15%** khi tra được `sub_id1` → `convert_results.group` → `zalo_groups.group_id` → `id_zl` đại lý. Đơn **Đã hủy** (trạng thái chứa “hủy”): chỉ cập nhật `order_status` trên các split, **giữ nguyên** `amount`. Đơn **Đã cộng tiền** chỉ refresh `order_placed_at` / `source_filename` — **không** chạy bước splits.

Response thêm: `import_batch_id` (uuid mỗi lần import có upsert đơn), `split_sync` (`splits_rows_upserted`, `orders_splits_status_only`, `splits_rows_deleted`).
